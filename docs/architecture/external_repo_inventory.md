...
| Gen-Searcher | Git repo; metadata read by `source_researcher` with per-command `safe.directory` | `main` | `e5078d31859bafee6b6b610f0cd40095cc72e2a4` | dirty | no top-level license; `Gen-DeepResearch-SFT/LICENSE` Apache-2.0, `Gen-DeepResearch-RL/LICENSE` MIT, vendored licenses vary | Do not copy wholesale. Interface ideas only unless exact subtree license is checked. |
| GenEvolve | Git repo; metadata read by `source_researcher` with per-command `safe.directory` | `main` | `23c847c559ccc0f95bbf4b3d8925898463822f4c` | dirty, including `LICENSE` | working-tree `LICENSE` is Apache-2.0 | Do not copy until dirty license/source state is reconciled. Prefer provenance and tool-pattern ideas. |
